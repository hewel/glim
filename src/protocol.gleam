...
fn decode_file_offer(input: String) -> Result(ClientEvent, DecodeError) {
  let decoder = {
    use to <- decode.field("to", decode.string)
    use client_offer_id <- decode.field("client_offer_id", decode.string)
    use name <- decode.field("name", decode.string)
    use size <- decode.field("size", decode.int)
    use mime_type <- decode.field("mime_type", decode.string)
    decode.success(#(to, client_offer_id, name, size, mime_type))
  }
  ...
  let #(to, client_offer_id, name, size, mime_type) = fields
  use valid_to <- result.try(
    validate_payload(validation.validate_device_id(to)),
  )
  use valid_client_offer_id <- result.try(
    validate_payload(validation.validate_transfer_id(client_offer_id)),
  )
  use valid_name <- result.try(
  ...
  Ok(FileOffer(
    to: valid_to,
    client_offer_id: valid_client_offer_id,
    name: valid_name,
    size: valid_size,
...
pub fn encode_transfer_accepted(
  transfer_id: String,
  upload_url: String,
) -> String {
  json.object([
    #("type", json.string("transfer.accepted")),
...
pub fn encode_transfer_progress(
  transfer_id: String,
  phase: String,
  bytes: Int,
  total: Int,
  ...
  json.object([
    #("type", json.string("transfer.progress")),
    #("transfer_id", json.string(transfer_id)),
...
pub fn encode_transfer_ready(
  transfer_id: String,
  download_url: option.Option(String),
) -> String {
  json.object([
    #("type", json.string("transfer.ready")),
    #("transfer_id", json.string(transfer_id)),
...
pub fn encode_transfer_done(transfer_id: String) -> String {
  encode_file_transfer_id("transfer.done", transfer_id)
}

pub fn encode_transfer_failed(transfer_id: String, reason: String) -> String {
  json.object([
    #("type", json.string("transfer.failed")),
    #("transfer_id", json.string(transfer_id)),
    #("reason", json.string(reason)),
  ])
  |> json.to_string
